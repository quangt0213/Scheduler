...
function validBreakWindows(shift, cfg){
  ...
  for(let t=Math.ceil(S/30)*30; t+bd<=E; t+=30){
  ...
  // post-rush (later-half) windows; slight preference for on-the-hour starts
  const midShift=(S+E)/2;
  const score=w=>{ const m=(w.s+w.e)/2; return Math.abs(m-midShift)*(m<midShift?3:1) + (w.s%60?20:0); };
