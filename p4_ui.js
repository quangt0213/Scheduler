...
// target staff for a slot, scaled from that day's sales intensity
...
App.cellClass = function(n, day, t){
  ...
  const target=App.targetStaff(day,t);
  const diff=n-target;
  if(diff<=-2) return "cell-r";
  if(diff===-1) return "cell-y";
  if(diff>=3) return "cell-b";
  return "cell-g";
};
...
App.renderHeatMap = function(){
  ...
  let html = "<tr><th>Time</th>"+DAYS.map(d=>"<th>"+DAY_LABEL[d]+"</th>").join("")+"<th>Sales intensity</th></tr>";
  for(const t of hrs){
    ...
    for(const d of DAYS){
      ...
      const visible = t < schedClose(d);
      const n = visible? staffAt(d,t) : 0;
      if(!visible && t>=schedClose(d)){ html+="<td class='cell-off'>–</td>"; continue; }
      const target=App.targetStaff(d,t);
      const inStore=t>=open&&t<toMin(state.config.storeHours[d].close);
      const show = (n===0 && t<open) ? "–" : n;
      html += "<td class='"+App.cellClass(n,d,t)+"' title='"+DAY_LABEL[d]+" "+fmt12(t)+": "+n+" staff"+(inStore?" / demand "+target:t<open?" (prep)":" (closing)")+"'>"+show+"</td>";
    }
